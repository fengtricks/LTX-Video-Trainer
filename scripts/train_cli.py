...
def generate_config(args, output_dir: Path) -> dict:
...
def setup_dataset(dataset_name: str, datasets_root: Path, training_data_dir: Path):
    ...
    # Clean and create training data directory
    ...
    # Check if dataset has precomputed data
...
def train_with_progress_callback(trainer: LtxvTrainer, db: JobDatabase, job_id: int):
    ...
    def progress_callback(step: int, total_steps: int, sampled_videos: list[Path] | None = None):
        """Update job progress in database."""
        progress_pct = (step / total_steps) * 100
        progress_msg = f"Step {step}/{total_steps} ({progress_pct:.1f}%)"

        # Get validation sample path if available
...
def main():
    ...
    training_data_dir = project_root / "training_data" / f"job_{args.job_id}" if args.job_id else project_root / "training_data"
    output_dir = project_root / "outputs" / f"lora_r{args.lora_rank}_job{args.job_id}" if args.job_id else project_root / "outputs" / f"lora_r{args.lora_rank}"

    # Set up database connection if job ID provided
    db = None
    if args.job_id:
    ...
    try:
        # Log in to HuggingFace if token provided
        ...
        # Train with progress callback if job ID provided
